...
export function generateSymbolDocument(symbol: XpedSymbol, uuid: string): string {
	...
	const s =
		symbol.version >= 54 ? (v: number) => Math.round(((v * zoom) / 254000) * 100) / 100 : (v: number) => Math.round(v * zoom * 0.1 * 100) / 100;
...
function emitPinsForPart(
	...
	for (const pin of pins) {
		...
		const pinLength = Math.round(Math.sqrt(Math.pow(bodyXs - tipXs, 2) + Math.pow(bodyYs - tipYs, 2)));
		...
		if (pin.pinNumbers.length > 0) {
			...
			const numValue = pin.pinNumbers.length > partIndex ? pin.pinNumbers[partIndex] : pin.pinNumbers.join(',');
			const numX = tipXs + 2;
			const numY = tipYs - 1;
...
function emitGraphic(lines: string[], graphic: XpedSymbolGraphic, s: (v: number) => number, zIndex: number, partTitle: string): number {
	switch (graphic.type) {
		case 'polyline':
		case 'polygon': {
		...
		case 'rect': {
			const polyId = nextSymId('e');
			const ticket = nextSymTicket();
			const x1 = s(graphic.x1);
			const y1 = s(-graphic.y1);
			const x2 = s(graphic.x2);
			const y2 = s(-graphic.y2);
			const pts = [
				`{"x":${x1},"y":${y1},"hashed":0}`,
				`{"x":${x2},"y":${y1},"hashed":0}`,
				`{"x":${x2},"y":${y2},"hashed":0}`,
		...
		case 'circle': {
			const polyId = nextSymId('e');
			const ticket = nextSymTicket();
			const cx = s(graphic.cx);
			const cy = s(-graphic.cy);
...
function filterPinsForPart(pins: XpedSymbolPin[], partIndex: number, partsCount: number, hetero: string[]): XpedSymbolPin[] {
...
export function generateSymbolSource(symbol: XpedSymbol): string {
