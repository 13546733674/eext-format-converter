...
export function generateFootprintBoilerplate(uuid: string, title: string, contentClient: string): string {
	...
	for (const [layerId, layerType, layerName, activeColor, inactiveColor, inactiveTrans] of layerEntries) {
		t++;
		lines.push(
			`{"type":"LAYER","ticket":${t},"id":"[\\\"LAYER\\\",${layerId}]","client":"${contentClient}"}||{"layerType":"${layerType}","layerName":"${layerName}","use":true,"show":true,"locked":false,"activeColor":"${activeColor}","activateTransparency":1,"inactiveColor":"${inactiveColor}","inactiveTransparency":${inactiveTrans}}|`,
		);
	}
	...
	lines.push(
		`{"type":"CANVAS","ticket":${canvasT},"id":"CANVAS","client":"${contentClient}"}||{"originX":0,"originY":0,"unit":"mm","gridXSize":10,"gridYSize":10,"snapXSize":0.393701,"snapYSize":0.393701,"gridType":"NONE","multiGridType":"NONE","highlightValue":0.5}|`,
	);
