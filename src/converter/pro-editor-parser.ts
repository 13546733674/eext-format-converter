...
import type {
  ...
  EeSymbolPinSettings, EeSymbolPinPath,
  EeSymbolRectangle, EeSymbolCircle, EeSymbolArc, EeSymbolEllipse,
  EeSymbolLine, EeSymbolPolyline, EeSymbolPolygon, EeSymbolPath,
  EeFootprint, EeFootprintInfo, EeFootprintBbox, EeFootprintPad,
  EeFootprintTrack, EeFootprintHole, EeFootprintVia,
...
function parseProSource(source: string): ProDoc {
  ...
  for (const line of lines) {
    ...
    switch (header.type) {
      case 'DOCHEAD': docHead = data; break;
      case 'CANVAS': canvas = data; break;
      case 'PART': parts.set(header.id, data); break;
      default:
    ...
    switch (elem.type) {
      case 'PIN': {
        ...
        sub.pins.push({
          ...
          name: {
            ...
            posX: nameAttr?.data.x || 0, posY: nameAttr?.data.y || 0,
          ...
          number: {
            ...
            posX: numAttr?.data.x || 0, posY: numAttr?.data.y || 0,
      ...
      case 'RECT':
        sub.rectangles.push({
          ...
          rx: d.radiusX || null, ry: d.radiusY || null,
          ...
          id: elem.id, isLocked: d.locked || false,
        });
        break;
      case 'CIRCLE':
        sub.circles.push({
          centerX: d.centerX, centerY: d.centerY, radius: d.radius,
          strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
          strokeStyle: d.strokeStyle ?? '',
          fillColor: d.fillStyle != null ? String(d.fillStyle) !== 'none' : false,
          id: elem.id, isLocked: d.locked || false,
        });
        break;
      case 'ELLIPSE':
        sub.ellipses.push({
          centerX: d.centerX || 0, centerY: d.centerY || 0,
      ...
      case 'LINE':
        sub.lines.push({
          x1: d.x1 ?? d.dotX1 ?? 0, y1: d.y1 ?? d.dotY1 ?? 0,
          x2: d.x2 ?? d.dotX2 ?? 0, y2: d.y2 ?? d.dotY2 ?? 0,
          strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
          strokeStyle: d.strokeStyle ?? '', fillColor: d.fillColor ?? '',
          id: elem.id, isLocked: d.locked || false,
        });
...
export function parseProFootprint(source: string): EeFootprint {
  ...
  for (const elem of doc.elements) {
    ...
    switch (elem.type) {
      ...
      case 'LINE':
        if (d.points) {
          const pts = String(d.points).split(/[\s,]+/).map(Number).filter(n => !isNaN(n));
    ...
    switch (elem.type) {
      ...
      case 'CIRCLE':
        footprint.circles.push({
          cx: d.centerX ?? 0, cy: d.centerY ?? 0, radius: d.radius ?? 0,
          strokeWidth: d.strokeWidth ?? '', layerId: d.layerId ?? d.layer ?? 0,
          id: elem.id, isLocked: d.locked ?? false,
        });
        break;
      case 'RECT':
        footprint.rectangles.push({
          x: d.dotX1 ?? d.x ?? 0, y: d.dotY1 ?? d.y ?? 0,
          width: d.width ?? Math.abs((d.dotX2 ?? 0) - (d.dotX1 ?? d.x ?? 0)),
          height: d.height ?? Math.abs((d.dotY2 ?? 0) - (d.dotY1 ?? d.y ?? 0)),
          strokeWidth: d.strokeWidth ?? '', id: elem.id,
          layerId: d.layerId ?? d.layer ?? 0, isLocked: d.locked ?? false,
        });
        break;
      case 'ARC':
        footprint.arcs.push({
          strokeWidth: d.strokeWidth ?? '', layerId: d.layerId ?? d.layer ?? 0,
          net: d.net ?? '', path: d.path ?? d.d ?? '',
          helperDots: '', id: elem.id, isLocked: d.locked ?? false,
        });
        break;
      case 'TEXT':
        footprint.texts.push({
          type: d.type ?? '', centerX: d.x ?? d.centerX ?? 0,
          centerY: d.y ?? d.centerY ?? 0, strokeWidth: d.strokeWidth ?? '',
          rotation: d.rotation ?? 0, miror: d.mirror ?? '',
          layerId: d.layerId ?? d.layer ?? 0, net: d.net ?? '',
          fontSize: d.fontSize ?? '', text: d.text ?? d.value ?? '',
          textPath: d.textPath ?? '', isDisplayed: d.display ?? d.isDisplayed ?? true,
          id: elem.id, isLocked: d.locked ?? false,
        });
        break;
      case 'COPPERAREA':
        footprint.copperAreas.push({
          ...
          clearanceWidth: d.clearanceWidth ?? '', fillStyle: d.fillStyle ?? '',
          id: elem.id, thermal: d.thermal ?? '',
          isKeepIsland: d.isKeepIsland ?? false, copperZone: d.copperZone ?? '',
          isLocked: d.locked ?? false,
        });
        break;
      case 'SOLIDREGION':
