...
import { EasyedaFootprintImporter } from './easyeda-importer';
import type { EeFootprint, EeFootprintPad, EeFootprintHole } from './models-easyeda';
import {
  RectanglePad, RoundPad, OblongPad, PolygonPad,
  RoundHole, SlotHole,
  PinSMDPadStack, PinThroughPadStack,
  XpeditionPin, XpeditionCell,
  PolylineShape, CirclePath, PolylinePath,
  AssemblyOutline, SilkscreenOutline, SolderMask, SolderPaste, PlacementOutline,
} from './models-xpedition';
import { eeUnitToTh, applyVerticalMirror, mapEasyedaLayerToXpedition } from './constants';
...
export class FootprintConverter {
  ...
  static fromEeFootprint(fp: EeFootprint): FootprintConverter {
  ...
  private _convertPadstacks(): void {
    ...
    for (const pad of this._easyedaFootprint.pads) {
      ...
      if (shapeUpper === 'RECT') {
        ...
        const largePoints = points.map(([px, py]) => [
          px * (largeWidth / width), py * (largeHeight / height),
        ] as [number, number]);
      ...
      if (pad.holeRadius > 0 || holePoints) {
        ...
        if (holePoints) {
          ...
          xpedHole = new SlotHole(
            `HOLE_${holeRadius * 2}x${holeLength}`,
            slotWidth, slotHeight, pad.isPlated,
          );
        } else {
          xpedHole = new RoundHole(
            `HOLE_${Math.round(eeUnitToTh(pad.holeRadius * 2) * 10000) / 10000}`,
            eeUnitToTh(pad.holeRadius * 2), pad.isPlated,
          );
        }
        ...
        xpedPadstack.setPads({
          topPad: xpedPad, bottomPad: xpedPad, internalPad: xpedPad,
          topSoldermaskPad: largePad, bottomSoldermaskPad: largePad,
          hole: xpedHole,
        });
        ...
        xpedPadstack.setPads({
          topPad: xpedPad, bottomPad: xpedPad,
          topSolderpastePad: xpedPad, bottomSolderpastePad: xpedPad,
          topSoldermaskPad: largePad, bottomSoldermaskPad: largePad,
        });
        if (!this._padstacks.has(padstackName)) {
          this._padstacks.set(padstackName, xpedPadstack);
