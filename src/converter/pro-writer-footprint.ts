...
import {
  generateFootprintBoilerplate, mmToEeUnit, mmToEeUnitInt,
} from './pro-layers';
import type {
  XpedCell, XpedPin, XpedPadstack, XpedPad, XpedHole,
  XpedCellOutline, XpedOutlineShape,
} from './hkp-parser';
...
function nextId(): string { return `e${++_elementId}`; }
function nextTicket(): number { return ++_ticket; }
...
export function generateFootprintSource(
  ...
  if (totalFills > 0) {
    lines.push(`{"type":"ELE_PLACEHOLDER","ticket":${nextTicket()},"id":"placeholder1","client":"${_client}"}||{"dataType":"FILL","max":${totalFills}}|`);
  ...
  if (outlineShapes.length > 0) {
    lines.push(`{"type":"ELE_PLACEHOLDER","ticket":${nextTicket()},"id":"placeholder2","client":"${_client}"}||{"dataType":"POLY","max":${outlineShapes.length}}|`);
  ...
  if (pinInfos.length > 0) {
    lines.push(`{"type":"ELE_PLACEHOLDER","ticket":${nextTicket()},"id":"placeholder3","client":"${_client}"}||{"dataType":"PAD","max":${pinInfos.length}}|`);
    for (const info of pinInfos) {
      const id = nextId();
  ...
  switch (shape.type) {
    case 'RECT_SHAPE': {
      if (shape.points && shape.points.length >= 2) {
        ...
        const x1 = mmToEeUnit(p1.x), y1 = mmToEeUnit(p1.y);
        const x2 = mmToEeUnit(p2.x), y2 = mmToEeUnit(p2.y);
