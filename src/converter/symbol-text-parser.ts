...
export function parseSymbolFile(content: string, fileName: string): XpedSymbol {
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    switch (cmd) {
      case 'V': {
        symbol.version = parseInt(parts[0]) || 0;
        break;
      }
      case 'K': {
        // K UnixTimestamp SymbolName
      ...
      case 'Y': {
        symbol.symbolType = parseInt(parts[0]) || 0;
        break;
      }
      case 'D': {
      ...
      case 'U': {
        ...
        if (keyVal) {
          ...
          if (key === 'PARTS') {
            symbol.partsCount = parseInt(val) || 1;
          }
        }
        break;
      }
      case 'P': {
        ...
        if (parts.length >= 8) {
          currentPin = {
            ...
            rotation: parseInt(parts[6]),
            inverted: parseInt(parts[7]),
            pinType: '',
            pinNumbers: [],
            label: '',
            labelX: 0, labelY: 0, labelSize: 0, labelRotation: 0,
            labelOrigin: 0, labelVisible: true,
      ...
      case 'A': {
        ...
        if (aKeyVal && currentPin) {
          if (aKeyVal[1] === 'PINTYPE') {
            ...
            if (parts.length > 5) {
              currentPin.pinNumberVisible = parseInt(parts[5]) !== 0;
            }
          }
        }
        break;
      ...
      case 'L': {
        ...
        if (parts.length >= 9 && currentPin) {
          ...
          currentPin.labelOrigin = parseInt(parts[4]) || 0;
          currentPin.labelVisible = parseInt(parts[6]) !== 0;
          ...
          currentPin.labelRotation = parseFloat(parts[3]) || 0;
          currentPin.labelOrigin = parseInt(parts[4]) || 0;
          currentPin.labelVisible = parseInt(parts[5]) !== 0;
