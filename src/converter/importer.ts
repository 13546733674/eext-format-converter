...
import { parsePadsFile, parseCellFile } from './hkp-parser';
import type { XpedPad, XpedHole, XpedPadstack, XpedCell } from './hkp-parser';
...
import { generateFootprintSource } from './pro-writer-footprint';
import { generateSymbolDocument } from './pro-writer-symbol';
...
export async function importXpeditionZip(
  ...
  zip.forEach((path) => { fileEntries.push(path); });
  ...
  for (const path of fileEntries) {
    ...
    if (lower.endsWith('.json') || lower.endsWith('.txt') || lower.endsWith('.md')
      || lower.endsWith('.xml') || lower.endsWith('.csv') || lower.endsWith('.log')
      || lower.endsWith('.elibu') || lower.endsWith('.elibz2') || lower.endsWith('.zip')
    ) {
      continue;
    }
  ...
  for (const part of allParts) {
    ...
    try {
      ...
      const symRefName = part.symbolRef.includes(':')
        ? part.symbolRef.split(':').pop()!
        : part.symbolRef;
      ...
      result.deviceItems!.push({ name: part.name || part.number, status: 'fail', message: String(e), uuid: '', symbolUuid: '', footprintUuid: '' });
    }
    doneItems++;
  }
  ...
  for (const part of allParts) {
    if (part.topCell && part.bottomCell && part.topCell !== part.bottomCell) {
      const devEntry = (Object.values(deviceData.devices) as any[]).find(
        (d: any) => d.display_title === (part.name || part.number)
      );
      if (devEntry) {
...
export async function filterImportResult(
  result: ImportResult,
  ...
  for (const fp of selFootprints) {
    ...
    deviceData.footprints[fp.uuid] = {
      uuid: fp.uuid, ticket: 1, updateTime: Date.now(), createTime: Date.now(),
      title: fp.name.toLowerCase(), display_title: fp.name, docType: 4,
    };
  }
  for (const sym of selSymbols) {
    libLines.push(sym.documentSource);
    deviceData.symbols[sym.uuid] = {
      uuid: sym.uuid, ticket: 1, updateTime: Date.now(), createTime: Date.now(),
      title: sym.name.toLowerCase(), display_title: sym.name, docType: 2,
    };
  }
  for (const dev of selDevices) {
    ...
    deviceData.devices[dev.uuid] = {
      uuid: dev.uuid, attributes: attrs, ticket: 1, updateTime: Date.now(),
      createTime: Date.now(), title: dev.name.toLowerCase(), display_title: dev.name,
      symbol_type: 2,
    };
  }
