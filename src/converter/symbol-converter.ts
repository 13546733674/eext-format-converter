...
import { EasyedaSymbolImporter } from './easyeda-importer';
import type { EeSymbol, EeSymbolSub, EeSymbolPin } from './models-easyeda';
import {
  XpeditionSymbol, SymbolPart,
  SymbolPinPosition, SymbolPin, SymbolLabel, SymbolAnnotation, SymbolPinGroup,
  SymbolShapeLine, SymbolShapeCircle, SymbolShapeArc,
} from './models-xpedition';
...
export class SymbolConverter {
  ...
  constructor(easyedaCadData: any) {
  ...
  static fromEeSymbol(sym: EeSymbol): SymbolConverter {
  ...
  private _calcPinPosition(pin: EeSymbolPin): { points: [number, number, number, number]; side: number } {
    ...
    if (path.includes('h')) {
      ...
      if (x1 > x2) {
        side = 3; // right
        if (rotation === 90) { x2 = x1; y2 = y1 - parseFloat(pathList[2]); side = 0; }
        else if (rotation === 270) { x2 = x1; y2 = y1 + parseFloat(pathList[2]); side = 1; }
      } else {
        side = 2; // left
        if (rotation === 90) { x2 = x1; y2 = y1 + parseFloat(pathList[2]); side = 0; }
        else if (rotation === 270) { x2 = x1; y2 = y1 - parseFloat(pathList[2]); side = 1; }
      ...
      if (y1 > y2) {
        side = 1; // bottom
        if (rotation === 90) { x2 = x1 - parseFloat(pathList[2]); y2 = y1; side = 2; }
        else if (rotation === 180) { side = 0; }
        else if (rotation === 270) { x2 = x1 + parseFloat(pathList[2]); y2 = y1; side = 3; }
      } else {
        side = 0; // top
        if (rotation === 90) { x2 = x1 + parseFloat(pathList[2]); y2 = y1; side = 3; }
        else if (rotation === 180) { side = 1; }
        else if (rotation === 270) { x2 = x1 - parseFloat(pathList[2]); y2 = y1; side = 2; }
      }
    }
  ...
  private _convertPinGroupsForSubpart(subpart: EeSymbolSub, partName: string): void {
    ...
    for (const pin of subpart.pins) {
      ...
      const symPos = new SymbolPinPosition(
        Math.round(points[2] - bbox.x), Math.round(bbox.y - points[3]),
        Math.round(points[0] - bbox.x), Math.round(bbox.y - points[1]),
      );
  ...
  private _convertShapesForSubpart(subpart: EeSymbolSub, partName: string): void {
    ...
    for (const line of subpart.lines) {
      this._xpeditionSymbol.addShape(
        new SymbolShapeLine(Math.round(line.x1 - bbox.x), Math.round(bbox.y - line.y1), Math.round(line.x2 - bbox.x), Math.round(bbox.y - line.y2)),
        partName,
      );
    }
  ...
  saveToFiles(): Record<string, string> {
    ...
    const generateSymbolString = (part: SymbolPart, partName?: string): string => {
      ...
      if (partName) {
        const hetero = Array.from(sym.parts.keys()).map(n => `(${n.replace(/\./g, '_')})`).join(',');
    ...
    if (sym.parts.size > 1) {
      ...
      for (const partName of sortedKeys) {
        const partNum = partName.includes('.') ? partName.split('.').pop() : '1';
