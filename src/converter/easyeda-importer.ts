...
import type {
  EeSymbol, EeSymbolInfo, EeSymbolBbox, EeSymbolSub, EeSymbolPin,
  EeSymbolPinSettings, EeSymbolPinDot, EeSymbolPinPath, EeSymbolPinName,
  EeSymbolPinNumber, EeSymbolPinDotBis, EeSymbolPinClock,
  EeSymbolRectangle, EeSymbolCircle, EeSymbolArc, EeSymbolEllipse,
  EeSymbolLine, EeSymbolPolyline, EeSymbolPolygon, EeSymbolPath,
  EeFootprint, EeFootprintInfo, EeFootprintBbox, EeFootprintLayer,
  EeFootprintPad, EeFootprintTrack, EeFootprintHole, EeFootprintVia,
  EeFootprintCircle, EeFootprintRectangle, EeFootprintArc,
  EeFootprintText, EeFootprintCopperArea, EeFootprintSolidRegion,
} from './models-easyeda';
import {
  makeEeSymbolBbox, makeEeSymbolPinSettings, makeEeSymbolPinPath,
  makeEeSymbolPinName, makeEeSymbolPinNumber,
} from './models-easyeda';
...
function addRectangle(data: string, sub: EeSymbolSub): void {
  ...
  sub.rectangles.push({
    posX: parseFloat(d.posX ?? '0'), posY: parseFloat(d.posY ?? '0'),
    rx: d.rx || null, ry: d.ry || null,
    width: parseFloat(d.width ?? '0'), height: parseFloat(d.height ?? '0'),
    strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
    strokeStyle: d.strokeStyle ?? '', fillColor: d.fillColor ?? '',
    id: d.id ?? '', isLocked: emptyLock(d.isLocked),
  });
}
...
function addCircle(data: string, sub: EeSymbolSub): void {
  ...
  sub.circles.push({
    centerX: parseFloat(d.centerX ?? '0'), centerY: parseFloat(d.centerY ?? '0'),
    radius: parseFloat(d.radius ?? '0'),
    strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
    strokeStyle: d.strokeStyle ?? '', fillColor: parseFill(d.fillColor),
    id: d.id ?? '', isLocked: emptyLock(d.isLocked),
  });
}
...
function addArc(data: string, sub: EeSymbolSub): void {
  ...
  sub.arcs.push({
    path: d.path ?? '', helperDots: d.helperDots ?? '',
    strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
    strokeStyle: d.strokeStyle ?? '', fillColor: parseFill(d.fillColor),
    id: d.id ?? '', isLocked: emptyLock(d.isLocked),
  });
...
function addEllipse(data: string, sub: EeSymbolSub): void {
  ...
  sub.ellipses.push({
    centerX: parseFloat(d.centerX ?? '0'), centerY: parseFloat(d.centerY ?? '0'),
    radiusX: parseFloat(d.radiusX ?? '0'), radiusY: parseFloat(d.radiusY ?? '0'),
    strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
    strokeStyle: d.strokeStyle ?? '', fillColor: parseFill(d.fillColor),
    id: d.id ?? '', isLocked: emptyLock(d.isLocked),
  });
...
function addLine(data: string, sub: EeSymbolSub): void {
  ...
  sub.lines.push({
    x1: parseFloat(d.x1 ?? '0'), y1: parseFloat(d.y1 ?? '0'),
    x2: parseFloat(d.x2 ?? '0'), y2: parseFloat(d.y2 ?? '0'),
    strokeColor: d.strokeColor ?? '', strokeWidth: d.strokeWidth ?? '',
    strokeStyle: d.strokeStyle ?? '', fillColor: d.fillColor ?? '',
    id: d.id ?? '', isLocked: emptyLock(d.isLocked),
  });
...
const SYMBOL_HANDLERS: Record<string, (data: string, sub: EeSymbolSub) => void> = {
  P: addPin, R: addRectangle, E: addEllipse, C: addCircle,
  A: addArc, PL: addPolyline, PG: addPolygon, PT: addPath, L: addLine,
};
...
export class EasyedaSymbolImporter {
  ...
  constructor(easyedaCpCadData: any) {
  ...
  getSymbol(): EeSymbol { return this.output; }
    ...
    if (eeData.subparts && eeData.subparts.length > 0) {
      for (const sub of eeData.subparts) {
        ...
        const newSub: EeSymbolSub = {
          ...
          pins: [], rectangles: [], circles: [], arcs: [],
          ellipses: [], polylines: [], polygons: [], paths: [], lines: [],
        };
...
const PAD_FIELDS = ['shape', 'centerX', 'centerY', 'width', 'height', 'layerId', 'net', 'number', 'holeRadius', 'points', 'rotation', 'id', 'holeLength', 'holePoint', 'isPlated', 'isLocked'];
...
const FTEXT_FIELDS = ['type', 'centerX', 'centerY', 'strokeWidth', 'rotation', 'miror', 'layerId', 'net', 'fontSize', 'text', 'textPath', 'isDisplayed', 'id', 'isLocked'];
const COPPER_FIELDS = ['strokeWidth', 'layerId', 'net', 'points', 'clearanceWidth', 'fillStyle', 'id', 'thermal', 'isKeepIsland', 'copperZone', 'isLocked'];
...
export class EasyedaFootprintImporter {
  ...
  getFootprint(): EeFootprint { return this.output; }
  ...
  private extractEasyedaData(eeDataStr: any, eeDataInfo: any, isSmd: boolean): EeFootprint {
    ...
    for (const line of eeDataStr.layers ?? []) {
      ...
      if (fields.length >= 6 && /\d/.test(fields[0])) {
        layers.push({
          layerId: parseInt(fields[0]), layerName: fields[1], layerColer: fields[2],
          isVisible: !!fields[3], isActive: !!fields[4], isConfig: !!fields[5],
        });
      }
    }
    ...
    const footprint: EeFootprint = {
      ...
      pads: [], tracks: [], holes: [], vias: [],
      circles: [], arcs: [], rectangles: [], texts: [],
      copperAreas: [], solidRegions: [],
    };
    ...
    for (const line of eeDataStr.shape ?? []) {
      ...
      if (desig === 'PAD') {
        ...
        footprint.pads.push({
          shape: d.shape ?? '', centerX: parseFloat(d.centerX ?? '0'), centerY: parseFloat(d.centerY ?? '0'),
          width: parseFloat(d.width ?? '0'), height: parseFloat(d.height ?? '0'),
          layerId: parseInt(d.layerId ?? '0'), net: d.net ?? '', number: d.number ?? '',
          holeRadius: parseFloat(d.holeRadius ?? '0'), points: d.points ?? '',
          rotation: d.rotation === '' ? 0 : parseFloat(d.rotation ?? '0'),
          id: d.id ?? '', holeLength: parseFloat(d.holeLength ?? '0'), holePoint: d.holePoint ?? '',
          isPlated: !!d.isPlated, isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
      } else if (desig === 'TRACK') {
        const d = zipToObj<any>(TRACK_FIELDS, fields);
        footprint.tracks.push({
          strokeWidth: parseFloat(d.strokeWidth ?? '0'), layerId: parseInt(d.layerId ?? '0'),
          net: d.net ?? '', points: d.points ?? '', id: d.id ?? '',
        ...
        footprint.holes.push({
          centerX: parseFloat(d.centerX ?? '0'), centerY: parseFloat(d.centerY ?? '0'),
          radius: parseFloat(d.radius ?? '0'), id: d.id ?? '',
          isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
      } else if (desig === 'VIA') {
        ...
        footprint.vias.push({
          centerX: parseFloat(d.centerX ?? '0'), centerY: parseFloat(d.centerY ?? '0'),
          diameter: parseFloat(d.diameter ?? '0'), net: d.net ?? '',
          radius: parseFloat(d.radius ?? '0'), id: d.id ?? '',
          isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
        ...
        footprint.circles.push({
          cx: parseFloat(d.cx ?? '0'), cy: parseFloat(d.cy ?? '0'),
          radius: parseFloat(d.radius ?? '0'), strokeWidth: parseFloat(d.strokeWidth ?? '0'),
          layerId: parseInt(d.layerId ?? '0'), id: d.id ?? '',
          isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
      } else if (desig === 'ARC') {
        ...
        footprint.arcs.push({
          strokeWidth: parseFloat(d.strokeWidth ?? '0'), layerId: parseInt(d.layerId ?? '0'),
          net: d.net ?? '', path: d.path ?? '', helperDots: d.helperDots ?? '',
          id: d.id ?? '', isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
      } else if (desig === 'RECT') {
        const d = zipToObj<any>(FRECT_FIELDS, fields);
        footprint.rectangles.push({
          x: parseFloat(d.x ?? '0'), y: parseFloat(d.y ?? '0'),
          width: parseFloat(d.width ?? '0'), height: parseFloat(d.height ?? '0'),
          strokeWidth: parseFloat(d.strokeWidth ?? '0'), id: d.id ?? '',
          layerId: parseInt(d.layerId ?? '0'),
          isLocked: d.isLocked === '' ? false : !!parseFloat(d.isLocked),
        });
      } else if (desig === 'TEXT') {
        const d = zipToObj<any>(FTEXT_FIELDS, fields);
        footprint.texts.push({
          type: d.type ?? '', centerX: parseFloat(d.centerX ?? '0'), centerY: parseFloat(d.centerY ?? '0'),
          strokeWidth: parseFloat(d.strokeWidth ?? '0'),
          rotation: d.rotation === '' ? 0 : parseFloat(d.rotation ?? '0'),
          miror: d.miror ?? '', layerId: parseInt(d.layerId ?? '0'), net: d.net ?? '',
          fontSize: parseFloat(d.fontSize ?? '0'), text: d.text ?? '', textPath: d.textPath ?? '',
          isDisplayed: d.isDisplayed === '' ? true : !!d.isDisplayed,
          id: d.id ?? '', isLocked: d.isLocked === '' ? false : !!d.isLocked,
        });
      } else if (desig === 'COPPERAREA') {
        const d = zipToObj<any>(COPPER_FIELDS, fields);
        footprint.copperAreas.push({
          strokeWidth: parseFloat(d.strokeWidth ?? '0'), layerId: parseInt(d.layerId ?? '0'),
          net: d.net ?? '', points: d.points ?? '',
          clearanceWidth: parseFloat(d.clearanceWidth ?? '0'), fillStyle: d.fillStyle ?? '',
          id: d.id ?? '', thermal: d.thermal ?? '',
          isKeepIsland: !!d.isKeepIsland, copperZone: [], isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
      } else if (desig === 'SOLIDREGION') {
        const d = zipToObj<any>(SOLID_FIELDS, fields);
        footprint.solidRegions.push({
          layerId: parseInt(d.layerId ?? '0'), net: d.net ?? '',
          points: d.points ?? '', type: d.type ?? '',
          id: d.id ?? '', isLocked: d.isLocked !== '' ? !!d.isLocked : false,
        });
      }
    }
