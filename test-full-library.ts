...
import type { XpedPad, XpedHole, XpedPadstack, XpedCell } from './src/converter/hkp-parser';
...
async function main() {
  ...
  let fpOk = 0, fpFail = 0, fpSkip = 0;
  let symOk = 0, symFail = 0, symSkip = 0;

  // 5a. Footprints — build name→uuid map
  const cellNameToUuid = new Map<string, string>();
  for (const cell of allCells) {
    if (cell.pins.length === 0) { fpSkip++; continue; }
    try {
      const uuid = genUUID();
      const source = generateFootprintSource(cell, psMap, padMap, holeMap, uuid);
      libLines.push(source);
      deviceData.footprints[uuid] = {
        uuid, ticket: 1, updateTime: Date.now(), createTime: Date.now(),
        title: cell.name.toLowerCase(), display_title: cell.name, docType: 4,
      };
  ...
  for (const [name, files] of symGroups) {
    try {
      ...
      if (symbols.length === 0) { symSkip++; continue; }
      ...
      deviceData.symbols[uuid] = {
        uuid, ticket: 1, updateTime: Date.now(), createTime: Date.now(),
        title: name.toLowerCase(), display_title: name, docType: 2,
      };
